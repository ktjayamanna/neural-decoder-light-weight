...
void CtcWfstBeamSearch::SetOpt(CtcWfstBeamSearchOptions& opts) {
  opts_ = opts;
  decodable_ = DecodableTensorScaled(opts.acoustic_scale);
}
...
void CtcWfstBeamSearch::Search(const torch::Tensor& logp) {
  ...
  if (decoded_frames_mapping_.size() > 0) {
    ...
    likelihood_[0] = std::make_pair(-weight.Value1(), -weight.Value2());
  }
}
...
void CtcWfstBeamSearch::FinalizeSearch() {
  ...
  if (decoded_frames_mapping_.size() > 0) {
    ...
    if (opts_.nbest == 1) {
      ...
      fst::ShortestPath(lat_, &nbest_lat, opts_.nbest);
    ...
    for (int i = 0; i < nbest; i++) {
      ...
      likelihood_[i] = std::make_pair(-weight.Value1(), -weight.Value2());
    }
  }
}
