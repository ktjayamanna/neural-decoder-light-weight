...
#include "utils/logging_compatibility.h"
...
class CtcPrefixBeamSearch : public SearchInterface {
 public:
  ...
  const std::vector<std::pair<float, float> >& likelihood() const { return likelihood_; }
  ...
  const std::vector<std::pair<float, float> >& Likelihood() const override { return likelihood_; }
  const std::vector<std::vector<int>>& Times() const override { return times_; }
  const kaldi::Lattice& Lattice() const override { return lat_; }
